...
- Don't refactor what isn't broken. Precedence: when editing existing files, local
  style wins over the style rules; the rules govern new files.
- Noticed unrelated dead code or a bug? Mention it in the response — don't fix or
  delete silently.
...
- Genuinely blocked by ambiguity → ask; otherwise pick a sensible default and proceed.
...
- Never hardcode secrets/tokens in code, tests, or logs — env/config only; mask
...
(the `rules-review` skill audits the whole set). Never edit `~/.claude/rules`
